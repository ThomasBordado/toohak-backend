...
describe('adminQuizList testing', () => {
  ...
  describe('Unsuccessful Cases', () => {
    test('Invalid AuthUserId', () => {
      ...
      expect(adminQuizList(user.sessionId + 1)).toStrictEqual({ error: expect.any(String) });
    });
  });
  describe('Successful Cases', () => {
    ...
    test('One quiz owned', () => {
      ...
      expect(adminQuizList(user.sessionId)).toStrictEqual({ quizzes: [{ quizId: quiz.quizId, name: 'My Quiz' }] });
    });
